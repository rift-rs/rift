...
compile-bin.%:
	...
	@if [ "$(BUILD)" = "release" ]; then bash ./dist/bin/compress.sh "$(BUILD)" "$(target_$*)" "$(strip_$*)"; fi
...
promote:
	...
	@docker buildx build \
		--platform linux/arm64,linux/amd64 \
		--tag ghcr.io/rift-rs/rift:$(TAG) \
		--tag ghcr.io/rift-rs/rift:latest \
		--build-arg HASH=$(HASH) \
